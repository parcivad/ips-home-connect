    ...
    function getToken( $url, $client_id, $client_secret ) {
        ...
        if ( data["authorize"]["code"] != null ) {
            ...
            if ( data["token"]["refresh_token"] != null && data["token"]["access_token"] != null) {
                $distance = time() - $this->getLastTokenCall();
                $limit = $this->getExpiresIn() - 3600;

                if ( $distance >= $limit ) {
                    $this->refreshToken( $url, $client_id, $client_secret, "");
                } else {
            ...
            if ( !isset($query["error"]) && !isset($query["status"]) ) {
                $this->write("access_token", $query["access_token"], false);
                $this->write("refresh_token", $query["refresh_token"], false);
                $this->write("id_token", $query["id_token"], false);
                $this->write("expires_in", $query["expires_in"], false);

                $this->write("token_type", $query["token_type"], false);
                $this->write("scope", $query["scope"], false);
                $this->write("last_token_call", time(), false);
